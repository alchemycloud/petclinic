...
public class User implements Serializable {
    ...
    private final Set<TenantUser> userTenants = new LinkedHashSet<>();
    ...
    public Set<TenantUser> getUserTenants() {
        return Collections.unmodifiableSet(userTenants);
    }

    public final void removeUserTenants(TenantUser item) {
        if (!Hibernate.isInitialized(this)) {
            return;
        }
        final User instance = (User) Hibernate.unproxy(this);
        if (Hibernate.isInitialized(instance.userTenants)) {
            final Set<TenantUser> restOfSet = new LinkedHashSet<>(instance.userTenants);
            restOfSet.remove(item);
            instance.userTenants.clear();
            instance.userTenants.addAll(restOfSet);
        }
    }

    public final void addUserTenants(TenantUser item) {
        if (!Hibernate.isInitialized(this)) {
            return;
        }
        final User instance = (User) Hibernate.unproxy(this);
