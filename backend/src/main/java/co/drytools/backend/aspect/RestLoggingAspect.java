...
public class RestLoggingAspect extends AbstractProfiledAspect {
    ...
    @Around(
            "(execution(public * co.drytools.backend.rest.*ApiResource.*(..))"
                    + ")"
                    + " && !execution(public * co.drytools.backend.rest.AuthenticationApiResource.*(..))")
    public Object doBasicProfiling(ProceedingJoinPoint pjp) throws Throwable {
        return logApiCall(pjp, WARN);
    }

    @Around("execution(public * co.drytools.backend.rest.AuthenticationApiResource.*(..))")
