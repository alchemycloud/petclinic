package co.aleksa.website.list;
...
public class PetsList extends AbstractComponent {
    ...
    public void setOwnerLastName(String value, Integer order) {
        final WebElement webElement = getElement().findElements(By.xpath(".//*[@data-qa='ownerLastName']")).get(order);
        if (OsValidator.isMac()) {
            webElement.sendKeys(Keys.COMMAND + "a");
        } else {
            webElement.sendKeys(Keys.CONTROL + "a");
    ...
    public void scrollToOwnerLastName(Integer order) {
        final WebElement webElement = getElement().findElements(By.xpath(".//*[@data-qa='ownerLastName']")).get(order);
        getTest().scrollToElement(webElement, false);
        AbstractUITest.sleep(100);
    }
